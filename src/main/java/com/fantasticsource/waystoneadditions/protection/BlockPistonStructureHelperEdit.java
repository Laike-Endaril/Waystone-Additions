...
public class BlockPistonStructureHelperEdit
{
    ...
    public BlockPistonStructureHelperEdit(World worldIn, BlockPos posIn, EnumFacing pistonFacing, boolean extending)
    {
        this.world = worldIn;
    ...
    public boolean canMove()
    {
        ...
        if (!BlockPistonBaseEdit.canPush(pistonPos, iblockstate, this.world, this.blockToMove, this.moveDirection, false, this.moveDirection))
        {
    ...
    private boolean addBlockLine(BlockPos origin, EnumFacing p_177251_2_)
    {
        ...
        if (iblockstate.getBlock().isAir(iblockstate, this.world, origin)) return true;
        if (!BlockPistonBaseEdit.canPush(pistonPos, iblockstate, this.world, origin, this.moveDirection, false, p_177251_2_)) return true;
        if (origin.equals(this.pistonPos) || this.toMove.contains(origin)) return true;
        ...
        else
        {
            while (block.isStickyBlock(iblockstate))
            {
                ...
                if (iblockstate.getBlock().isAir(iblockstate, this.world, blockpos) || !BlockPistonBaseEdit.canPush(pistonPos, iblockstate, this.world, blockpos, this.moveDirection, false, this.moveDirection.getOpposite()) || blockpos.equals(this.pistonPos))
                {
                    break;
                }
            ...
            while (true)
            {
                ...
                if (!BlockPistonBaseEdit.canPush(pistonPos, iblockstate, this.world, blockpos1, this.moveDirection, true, this.moveDirection) || blockpos1.equals(this.pistonPos))
                {
                    return false;
                }
